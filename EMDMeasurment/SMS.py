...
class SMS: #Set-Multiset-Sequence calculator
    ...
    def disclosure_calc(self, bk_type, uniq_act,measurement_type, bk_length, existence_based, simple_log, multiset_log, map_dict_chr_act):
        ...
        if bk_type == "set":
            ...
            for index, sub_seq in enumerate(cand_seq):
                ...
                if sum == 1:
                    matches_char = self.convert_char_to_act(matches[0],map_dict_chr_act)
                    cand_seq_char = self.convert_char_to_act(sub_seq,map_dict_chr_act)
                if sum != 0:
                    matches_list.append(1/sum)
                    sum_uniq += 1 / sum
                    if sum != 1:
