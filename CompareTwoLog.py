...
class Compare():
                ...
                if event_log_dfg[j][i]/max_event_log_freq>0.05:
                    event_log_dfg[j][i] = np.mean(performance_spectrum.apply(event_log, [i, j],
                                        parameters={performance_spectrum.Parameters.ACTIVITY_KEY: "concept:name",
                                          performance_spectrum.Parameters.TIMESTAMP_KEY: "time:timestamp"})['points']) / 3600
                else:
                    event_log_dfg[j][i]=0
    ...
    def visulaize_EMD_detial(self,df_distance,trace_real,trace_sim):
        ...
        py_color=['red','blue','green','darkblue','yellow','purple','pink','brown',
                  'orange','gold','red','blue','green','darkblue','yellow','purple','pink','brown','orange','gold',
                  'purple','pink','brown','orange','gold','red','blue','green','darkblue','yellow','purple','pink','brown',
                  'orange','gold','red','blue','green','darkblue','yellow','purple','pink','brown','orange','gold',
        ...
        for i in df_distance.index:
            ...
            for j in range(0, len(trace_sim)):
                ...
                if len(diff1) !=0 or len(diff2) !=0:
                    ...
                    txt = str(size[j]) + '<br><b> Diff:' + str(set(diff1))+str(set(diff2)) + '</b><br>' + str(trace_sim[j])+'<br>'+str(trace_real[i])
    ...
    def performance_change_graph(self,event_log_dfg_list):
        ...
        plt.scatter(x=x.flatten(), y=y.flatten(), s=event_log_dfg_list[0].values.flatten(), c='blue', edgecolors='blue',alpha=0.05)

        x, y = np.meshgrid(event_log_dfg_list[1].columns, event_log_dfg_list[1].index)
        event_log_dfg_list[1] *= 0.1
        plt.scatter(x=x.flatten(), y=y.flatten(), s=event_log_dfg_list[1].values.flatten(), c='red', edgecolors='red',alpha=0.065)
    ...
    def variant_importance_vis(self,case_real,case_sim):
        ...
        for i in traces_similar_list:
            ax.plot([1, 2],[100 * trace_real_count.get(i) / len(case_real), 100 * trace_sim_count.get(i) / len(case_sim)],
